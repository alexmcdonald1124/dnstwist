...
class DomainFuzz():
    ...
    def __validate_domain(self, domain):
        if len(domain) == len(domain) and domain != domain:
            return False
        allowed = re.compile(
            '(?=^.{4,253}$)(^((?!-)[a-zA-Z0-9-]{1,63}(?<!-)\.)+[a-zA-Z]{2,63}\.?$)', re.IGNORECASE)
        return allowed.match(domain)
    ...
    def generate(self):
        ...
        if self.tld != 'com' and '.' not in self.tld:
            self.domains.append(
                {'fuzzer': 'Various', 'domain-name': self.domain + '-' + self.tld + '.com'})
    ...
    def run(self):
        while not self.kill_received:
            ...
            if self.option_extdns:
                ...
                try:
                    domain[
                        'dns-ns'] = self.answer_to_list(resolv.query(domain['domain-name'], 'NS'))
                except DNSException:
                    pass
                ...
                if 'dns-ns' in domain or len(domain['domain-name'].split('.')) > 1:
                    try:
                        domain[
                            'dns-a'] = self.answer_to_list(resolv.query(domain['domain-name'], 'A'))
                    except DNSException:
                        pass

                    try:
                        domain[
                            'dns-aaaa'] = self.answer_to_list(resolv.query(domain['domain-name'], 'AAAA'))
                    except DNSException:
                        pass

                    try:
                        domain[
                            'dns-mx'] = self.answer_to_list(resolv.query(domain['domain-name'], 'MX'))
                    except DNSException:
                        pass
            else:
                try:
            ...
            if self.option_whois:
                if 'dns-ns' in domain or 'dns-a' in domain:
                    try:
                        ...
                        domain[
                            'whois-created'] = str(whoisdb.creation_date).split(' ')[0]
                        domain[
                            'whois-updated'] = str(whoisdb.last_updated).split(' ')[0]
                    except Exception:
                        pass
                ...
                if 'dns-a' in domain:
                    try:
                        ...
                        if req.status_code / 100 == 2:
                            domain[
                                'ssdeep-score'] = ssdeep.compare(self.ssdeep_orig, ssdeep_fuzz)
...
class ThreatConnect():
    ...
    def __init__(self):
        self.tc_api_access_id = config_file['tc_api_access_id']
        self.tc_api_secret_key = config_file['tc_api_secret_key']
        self.tc_owner = config_file['tc_owner']
        self.tc_api_base_url = config_file['tc_api_base_url']


    def batch_commit(self, domains):

        dst_owner = self.tc_api_base_url

        dst_tc = ThreatConnect(self.tc_api_access_id, self.tc_api_secret_key, self.tc_owner, self.tc_api_base_url)
...
def generate_csv(domains):
    ...
    for domain in domains:
        output += '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % (domain.get('fuzzer'),
                                                       domain.get(
                                                           'domain-name').encode('idna'),
                                                       one_or_all(
                                                           domain.get('dns-a', [''])),
                                                       one_or_all(domain.get(
                                                           'dns-aaaa', [''])),
                                                       one_or_all(
                                                           domain.get('dns-mx', [''])),
                                                       one_or_all(
...
def generate_cli(domains):
    ...
    for domain in domains:
        ...
        if 'dns-ns' in domain:
            info += '%sNS:%s%s%s ' % (FG_YEL, FG_CYA,
                                      one_or_all(domain['dns-ns']), FG_RST)

        if 'dns-mx' in domain:
            if 'mx-spy' in domain:
                info += '%sSPYING-MX:%s%s' % (FG_YEL,
                                              domain['dns-mx'][0], FG_RST)
            else:
                info += '%sMX:%s%s%s ' % (FG_YEL, FG_CYA,
                                          one_or_all(domain['dns-mx']), FG_RST)

        if 'banner-http' in domain:
            info += '%sHTTP:%s"%s"%s ' % (FG_YEL,
                                          FG_CYA, domain['banner-http'], FG_RST)

        if 'banner-smtp' in domain:
            info += '%sSMTP:%s"%s"%s ' % (FG_YEL,
                                          FG_CYA, domain['banner-smtp'], FG_RST)
        ...
        if 'whois-created' in domain and 'whois-updated' in domain:
            if domain['whois-created'] == domain['whois-updated']:
                info += '%sCreated/Updated:%s%s%s ' % (
                    FG_YEL, FG_CYA, domain['whois-created'], FG_RST)
            else:
                if 'whois-created' in domain:
                    info += '%sCreated:%s%s%s ' % (FG_YEL,
                                                   FG_CYA, domain['whois-created'], FG_RST)
                if 'whois-updated' in domain:
                    info += '%sUpdated:%s%s%s ' % (FG_YEL,
                                                   FG_CYA, domain['whois-updated'], FG_RST)

        if 'ssdeep-score' in domain:
            if domain['ssdeep-score'] > 0:
                info += '%sSSDEEP:%d%%%s ' % (FG_YEL,
                                              domain['ssdeep-score'], FG_RST)
        ...
        output += '%s%s%s %s %s\n' % (FG_BLU, domain['fuzzer'].ljust(
            width_fuzzer), FG_RST, domain['domain-name'].ljust(width_domain), info)
...
def main():
    ...
    parser.add_argument('-f', '--format', type=str, choices=[
                        'cli', 'csv', 'json', 'idle'], default='cli', help='output format (default: cli)')
    ...
    parser.add_argument('-tc', '--whois', action='store_true',
                        help='Import domain data into ThreatConnect')
...
''' % __version__ + FG_RST + ST_RST)
    ...
    if args.ssdeep and MODULE_SSDEEP and MODULE_REQUESTS:
        ...
        try:
            req = requests.get(url.get_full_uri(), timeout=REQUEST_TIMEOUT_HTTP, headers={
                               'User-Agent': 'Mozilla/5.0 (dnstwist)'})
